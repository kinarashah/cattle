...
/**
 * Programmatically delete purged database rows after they reach a configurable
 * age.
 */
public class TableCleanup extends AbstractJooqDao implements Task {
    ...
    public static final DynamicLongProperty MAIN_TABLES_AGE_LIMIT_SECONDS = ArchaiusUtil
            .getLong("main_tables.purge.after.seconds");
    public static final DynamicLongProperty PROCESS_INSTANCE_AGE_LIMIT_SECONDS = ArchaiusUtil
            .getLong("process_instance.purge.after.seconds");
    public static final DynamicLongProperty EVENT_AGE_LIMIT_SECONDS = ArchaiusUtil
            .getLong("events.purge.after.seconds");
    public static final DynamicLongProperty AUDIT_LOG_AGE_LIMIT_SECONDS = ArchaiusUtil
            .getLong("audit_log.purge.after.seconds");
    public static final DynamicLongProperty SERVICE_LOG_AGE_LIMIT_SECONDS = ArchaiusUtil
            .getLong("service_log.purge.after.seconds");
    public static final String MOUNT_DELETE_QUERY = "delete m " + " from mount as m " + " join ( " + "    select mm.id "
            + "    from mount as mm " + "    join instance as i on mm.instance_id = i.id "
            + "    join volume as v on mm.volume_id = v.id " + "    where i.state = 'purged' "
            + "    and i.removed < ? "
            + "    and (v.data like '%\"isHostPath\":true%' or v.data like '%\"driver\":\"local\"%')" + "    limit ? "
            + " ) mx on m.id = mx.id";
    public static final String SECRET_CLEANUP_VSPM_QUERY = "delete s " + " from volume_storage_pool_map as s "
            + " join ( " + "   select vv.id " + "   from volume as vv "
            + "   where vv.data like '%\\\"driver\\\":\\\"rancher-secrets\\\"%' "
            + "   and created < date_sub(utc_timestamp(), INTERVAL 1 HOUR) " + "   and vv.state = \"inactive\" "
            + "   and vv.instance_id is NULL " + "   and id not in("
            + "       select volume_id from mount as m where m.state != \"removed\" or m.state != \"purged\""
            + "       )" + "   )vx on vx.id = s.volume_id";

    public static final String SECRET_CLEAUP_VOLUME_QUERY = "delete v " + " from volume as v " + " join ("
            + "   select vv.id from volume as vv " + "   where vv.data like '%\\\"driver\\\":\\\"rancher-secrets\\\"%' "
            + "   and created < date_sub(utc_timestamp(), INTERVAL 1 HOUR) and " + "   vv.state = \"inactive\" and "
            + "   vv.instance_id is NULL and " + "   id not in("
            + "       select volume_id from mount as m where m.state != \"removed\" or m.state != \"purged\""
            + "       )" + " ) vx on vx.id = v.id";
    ...
    private void cleanupServiceEventTable(Date cutoff) {
        ResultQuery<Record1<Long>> ids = create().select(SERVICE_EVENT.ID).from(SERVICE_EVENT)
                .where(SERVICE_EVENT.CREATED.lt(cutoff)).and(SERVICE_EVENT.STATE.eq(CommonStatesConstants.CREATED))
                .limit(QUERY_LIMIT_ROWS.getValue());

        List<Long> toDelete = null;
        int rowsDeleted = 0;
        while ((toDelete = ids.fetch().into(Long.class)).size() > 0) {
            rowsDeleted += create().delete(SERVICE_EVENT).where(SERVICE_EVENT.ID.in(toDelete)).execute();
        }
    ...
    private void cleanupExternalHandlerExternalHandlerProcessMapTables(Date cutoff) {
        ResultQuery<Record1<Long>> ids = create().select(EXTERNAL_HANDLER_EXTERNAL_HANDLER_PROCESS_MAP.ID)
                .from(EXTERNAL_HANDLER_EXTERNAL_HANDLER_PROCESS_MAP).join(EXTERNAL_HANDLER)
                .on(EXTERNAL_HANDLER_EXTERNAL_HANDLER_PROCESS_MAP.EXTERNAL_HANDLER_ID.eq(EXTERNAL_HANDLER.ID))
                .where(EXTERNAL_HANDLER.REMOVED.lt(cutoff)).limit(QUERY_LIMIT_ROWS.getValue());
    ...
    private void cleanupLabelTables(Date cutoff) {
        ResultQuery<Record1<Long>> ids = create().select(INSTANCE_LABEL_MAP.ID).from(INSTANCE_LABEL_MAP).join(INSTANCE)
                .on(INSTANCE_LABEL_MAP.INSTANCE_ID.eq(INSTANCE.ID)).where(INSTANCE.REMOVED.lt(cutoff))
                .limit(QUERY_LIMIT_ROWS.getValue());
        List<Long> toDelete = null;
        int ilmRowsDeleted = 0;
        while ((toDelete = ids.fetch().into(Long.class)).size() > 0) {
            ilmRowsDeleted += create().delete(INSTANCE_LABEL_MAP).where(INSTANCE_LABEL_MAP.ID.in(toDelete)).execute();
        }

        ids = create().select(HOST_LABEL_MAP.ID).from(HOST_LABEL_MAP).join(HOST).on(HOST_LABEL_MAP.HOST_ID.eq(HOST.ID))
                .where(HOST.REMOVED.lt(cutoff)).limit(QUERY_LIMIT_ROWS.getValue());

        int hlmRowsDeleted = 0;
        while ((toDelete = ids.fetch().into(Long.class)).size() > 0) {
            hlmRowsDeleted += create().delete(HOST_LABEL_MAP).where(HOST_LABEL_MAP.ID.in(toDelete)).execute();
        }

        ids = create().select(LABEL.ID).from(LABEL).leftOuterJoin(INSTANCE_LABEL_MAP)
                .on(LABEL.ID.eq(INSTANCE_LABEL_MAP.LABEL_ID)).leftOuterJoin(HOST_LABEL_MAP)
                .on(LABEL.ID.eq(HOST_LABEL_MAP.LABEL_ID)).where(INSTANCE_LABEL_MAP.ID.isNull())
                .and(HOST_LABEL_MAP.ID.isNull()).and(LABEL.CREATED.lt(cutoff)).limit(QUERY_LIMIT_ROWS.getValue());

        int labelRowsDeleted = 0;
        while ((toDelete = ids.fetch().into(Long.class)).size() > 0) {
            labelRowsDeleted += create().delete(LABEL).where(LABEL.ID.in(toDelete)).execute();
        }
    ...
    private void cleanup(String name, List<CleanableTable> tables, Date cutoffTime) {
        ...
        for (CleanableTable table : tables) {
            if (table.referenceCheckOnly) {
                // This table is just for reference checking when cleaning other tables. Don't
                // attempt to delete any rows from this table
                continue;
            }
            ...
            ResultQuery<Record1<Long>> idsToDeleteQuery = create().select(idField).from(table.table)
                    .where(removeField.lt(cutoffTime).and(idField.gt(DSL.param(ID_GREATER_THAN_PARAM, 0L))))
                    .orderBy(idField).limit(QUERY_LIMIT_ROWS.getValue());
            ...
            while (idsToDeleteResult.size() > 0) {
                ...
                if (idsToDelete.size() > 0) {
                    try {
                        int rowsDeleted = create().delete(table.table).where(idField.in(idsToDelete)).execute();
                        table.addRowsDeleted(rowsDeleted);
                        log.info("Deleted {} unneeded rows from {} table", idsToDelete.size(), table.table);
            ...
            if (idsThatCantBeDeleted.size() > 0) {
                ...
                log.info("Skipped {} where id in {}", table.table, idsThatCantBeDeleted);
            }
        }
        ...
        for (CleanableTable table : tables) {
            if (table.getRowsDeleted() > 0) {
                buffDeleted.append(table.table.getName()).append("=").append(table.getRowsDeleted()).append(" ");
                deletedActivity = true;
            }
            if (table.getRowsSkipped() > 0) {
                buffSkipped.append(table.table.getName()).append("=").append(table.getRowsSkipped()).append(" ");
    ...
    /**
     * Sorts a list of tables by their primary key references such that tables may
     * be cleaned in an order that doesn't violate any key constraints.
     *
    ...
    private static List<CleanableTable> getProcessInstanceTables() {
        List<CleanableTable> tables = Arrays.asList(CleanableTable.from(ProcessExecutionTable.PROCESS_EXECUTION),
    ...
    private static List<CleanableTable> getEventTables() {
        List<CleanableTable> tables = Arrays.asList(CleanableTable.from(ContainerEventTable.CONTAINER_EVENT));
        return sortByReferences(tables);
    }
    ...
    private static List<CleanableTable> getOtherTables() {
        List<CleanableTable> tables = Arrays.asList(CleanableTable.from(AccountTable.ACCOUNT),
                CleanableTable.from(AgentTable.AGENT), CleanableTable.from(AuthTokenTable.AUTH_TOKEN),
                CleanableTable.from(BackupTable.BACKUP), CleanableTable.from(BackupTargetTable.BACKUP_TARGET),
                ...
                CleanableTable.from(HostTable.HOST), CleanableTable.from(HostIpAddressMapTable.HOST_IP_ADDRESS_MAP),
                ...
                CleanableTable.from(InstanceLinkTable.INSTANCE_LINK), CleanableTable.from(IpAddressTable.IP_ADDRESS),
                CleanableTable.from(IpAddressNicMapTable.IP_ADDRESS_NIC_MAP), CleanableTable.from(LabelTable.LABEL),
                CleanableTable.from(MachineDriverTable.MACHINE_DRIVER), CleanableTable.from(MountTable.MOUNT),
                CleanableTable.from(NetworkTable.NETWORK), CleanableTable.from(NetworkDriverTable.NETWORK_DRIVER),
                CleanableTable.from(NicTable.NIC), CleanableTable.from(PhysicalHostTable.PHYSICAL_HOST),
                CleanableTable.from(PortTable.PORT), CleanableTable.from(ProjectMemberTable.PROJECT_MEMBER),
                CleanableTable.from(ResourcePoolTable.RESOURCE_POOL), CleanableTable.from(ServiceTable.SERVICE),
                ...
                CleanableTable.from(ServiceIndexTable.SERVICE_INDEX), CleanableTable.from(SnapshotTable.SNAPSHOT),
                CleanableTable.from(StackTable.STACK), CleanableTable.from(StorageDriverTable.STORAGE_DRIVER),
                CleanableTable.from(StoragePoolTable.STORAGE_POOL),
                CleanableTable.from(StoragePoolHostMapTable.STORAGE_POOL_HOST_MAP),
                CleanableTable.from(SubnetTable.SUBNET), CleanableTable.from(TaskInstanceTable.TASK_INSTANCE),
                CleanableTable.from(UserPreferenceTable.USER_PREFERENCE), CleanableTable.from(VolumeTable.VOLUME),
                ...
                // These tables are cleaned through specialized logic but we need to keep them
                // in the "other" list so that they
                ...
                CleanableTable.from(ScheduledUpgradeTable.SCHEDULED_UPGRADE), CleanableTable.from(SecretTable.SECRET),
                CleanableTable.from(ZoneTable.ZONE), CleanableTable.forReference(AuditLogTable.AUDIT_LOG),
                CleanableTable.forReference(ServiceLogTable.SERVICE_LOG),
                CleanableTable.forReference(ProcessInstanceTable.PROCESS_INSTANCE));
        /*
         * The most offending tables never set remove_time service_event
         * external_handler_external_handler_process_map instance_label_map mount
         * instance_link
         */
        return sortByReferences(tables);
    }

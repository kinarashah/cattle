...
public class ExternalServiceEventCreate extends AbstractDefaultProcessHandler {
    ...
    void createService(ExternalEvent event, Map<String, Object> serviceData) {
        ...
        for(Service existingSvc : existingSvcs) {
            if(existingSvc!=null && !existingSvc.getExternalId().equals(event.getExternalId())) {
                log.info("Deleting existing kubernetes service {} with externalId {} and accountId {}",
                            existingSvc.getName(), existingSvc.getExternalId(), existingSvc.getAccountId());
                objectProcessManager.scheduleStandardProcess(StandardProcess.REMOVE, existingSvc, null);
            }
        }

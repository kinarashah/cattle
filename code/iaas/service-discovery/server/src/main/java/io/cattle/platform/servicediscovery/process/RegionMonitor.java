    ...
    public void run() {
        ...
        for(AccountLink link : accountLinks) {
            ...
            try {
                ...
                log.info("Failed to monitor account link for %s - %s", link.getId(), ex);
            }
        }
        List<Account> accounts = objectManager.find(Account.class, ACCOUNT.REMOVED, new Condition(ConditionType.NULL));
        for(Account account : accounts) {
            regionService.reconcileExternalLinks(account.getId());
        }
    }
    
    protected void delete(AccountLink link) {
